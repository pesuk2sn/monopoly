...
def vanglaFunk(i,mängija_asetus, vanglas, raha):
    mängulaud = ("Go","Jaama tänav" ,"Ühisfond","Soinaste tänav" ,"Tulumaks","Ülenurme rongijaam","Ravila tänav","Loos","Ringtee tänav" ,"Ilmatsalu tänav" ,"Vangla","Vabaduse pst" ,"Elektrivõrk","Sõbra tänav" ,"Kesk kaar" ,"Kirsi rongijaam","Kalevi tänav" ,"Ühisfond","F.R Kreutzvaldi tänav" ,"Vaksali tänav" ,"Tasuta parkimine","Pikk tänav" ,"Loos","Aardla tänav" ,"Puussepa tänav" ,"Aardla rongijaam","Puiestee tänav" ,"Kalda tee" ,"Veevärk","Võru tänav" ,"Lähed vangi","Turu tänav" ,"Narva maantee" ,"Ühisfond","Riia tänav" ,"Tartu rongijaam","Loos","Rüütli tänav" ,"Lisamaks","Raekoja plats")
    if i==3: #kui kolm korda pole mängija saanud duublit, siis  kolmas kord saab mängija vabaks
        ...
        if täring_1 == täring_2:
            print("Veeretasid duubli, pääsed vanglast!")
            vanglas=False
            mängija_asetus=mängulaud[täringuvise()+mängulaud.index(mängija_asetus)]
            print("Mängija on ruudul "+str(mängija_asetus))
            return (mängija_asetus, vanglas, i, raha)
        else:
            return (mängija_asetus, vanglas, i+1, raha)
...
def näita_krunti(krunt):#See väljastab mingi krundi kõik muutujad selles järjekorras:
        ...
        with open("Monopoli krundid.txt", "r", encoding="UTF-8") as f: #Avab faili lugemiseks
            ...
            while rida: #kuni on ridu, mida lugeda
                ...
                if rida.lower() == krunt.lower(): # Vaatab, kas sisestatud värv ühtib enniku nimega
...
["Rüütli tänav","Raekoja plats","tumesinised"]]
    ...
    if mängija_asetus == "Veevärk" or mängija_asetus == "Elektrivõrk": #Kui mängija on veevärgis või elektrivõrgus
        ...
        if "Veevärk" in vastase_vara and "Elektrivõrk" in vastase_vara: #kui vastasel on mõlemad kaardid
            ...
            print("Sattusite konkurendi kommunaalile, tasu on " + str((täring1 + täring2) * 4) + " eurot.")
...
def endgame(rent, mängija_vara, käsk): #See funktsioon rakendub siis, kui mängija peab maksma rohkem trahvi/renti, kui ta kontol on.
    ...
    järjend = []
    i = 1
    while i in range(len(mängija_vara)): #Selleks, et suured tähed ei segaks inputi
        järjend.append(mängija_vara[i].lower())
        i += 1
    ...
    while rent > mängija_vara[0]: #kui mängijal pole piisavalt raha, et renti maksta 
        ...
        if len(väljasta_tänavad(mängija_vara)) < 1: #kui mängijal pole krunte
            print("\nTeil on krundid otsas, kaotasite mängu.")
...
''').lower()
                ...
                while pandiotsus not in järjend and pandiotsus != "info":
                ...
                if pandiotsus in järjend:
                    mängija_vara = pandi_krunt(mängija_vara[järjend.index(pandiotsus)+1], mängija_vara) #pandib krundi, lisab hinna mängijale
                    del järjend[järjend.index(pandiotsus)]
                    print("Krunt panditud, teie kontol on nüüd " + str(mängija_vara[0]-rent) + " eurot.")
...
def müük(mängija_vara, vastase_vara, käik): #See on käsk, mis rakendub siis, kui üks mängija tahab teisele oma kaarti müüa.
    #See tõstab elemendi ühest järjendist teise ja saadab valitud hulga raha teisest järjendist esimesse. See tagastab mängija vara ja ta konkurendi vara järjendid.
        ...
        i = 1
        while i in range(len(mängija_vara)): #ostib, kas mängijal on see kaart
            if kaart.lower() == mängija_vara[i].lower():
                u = i
                kas_kaart_on_mängijal = True
            i += 1
            
        if kas_kaart_on_mängijal == True:
            try: #kui hind pole int
                if vastase_vara[0] >= hind:
                    ...
                    if vastus.lower() == "jah": #teeb vahetuse
                        mängija_vara[0] += hind
                        vastase_vara[0] -= hind
                        vastase_vara.append(mängija_vara[u])
                        del mängija_vara[u]
...
2. krunt:
    ...
    kasutage käsku "värvid"
    
3. info mängija 1/2:
    Kui soovite teada, kui palju raha on mingil mängijal ning mis kaardid neil on. Nimed on "mängija 1" ja "mängija 2".
    
4. lõpp:
    Kui soovite oma käigu üle anda. Võite ka algus kirjutada.
    
5. pandi:
    Kui soovite ningi oma krundi pangale pantida. Saate selle eest pool ostuhinnast.
    Täpsemaks infoks kasutage käsku "krunt"
